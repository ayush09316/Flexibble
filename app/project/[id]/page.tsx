...
const Project = async ({ params: { id } }: { params: { id: string } }) => {
  ...
  if (!result?.project) return <p>Failed to fetch information</p>;
  const projectDetails = result?.project;
  const renderLink = () => `/project/${projectDetails?.createdBy?.id}`;

  return (
    <Modal>
      <section className=" flexBetween gap-y-8 max-w-[67rem] max-xs:flex-col w-full">
            ...
            <section className="mt-14 bg-slate-300 rounded-2xl drop-shadow-lg">
                <Image
